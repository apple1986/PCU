...
class PCE(nn.Module):
    def __init__(self, k, s, classes):
         ...
         self.a2 = nn.Conv2d(classes * self.gps, self.dim // 16, 1, padding=0)
         self.a3 = nn.ReLU(inplace=True)
         self.a4 = nn.Conv2d(self.dim // 16, classes * self.gps, 1, padding=0, bias=True)
         self.a5 = nn.Sigmoid()
...
class FastSCNN_SRFFA(nn.Module):
    def __init__(self, classes, channels, aux=True, **kwargs):
        super(FastSCNN_SRFFA, self).__init__()
        self.aux = aux
        self.learning_to_downsample = LearningToDownsample(32, 48, 64)
        self.global_feature_extractor = GlobalFeatureExtractor(64, [64, 96, 128], 128, 6, [3, 3, 3])
        self.feature_fusion = FeatureFusionModule(64, 128, 128)
        self.classifier = Classifer(128, classes)
        self.SR = PCE(k=3, s=8, classes=classes)
        if self.aux:
...
if __name__ == '__main__':
    ...
    model = FastSCNN_SRFFA(classes=19).to(device)
